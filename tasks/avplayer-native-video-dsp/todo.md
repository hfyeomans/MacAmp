...
- [x] 8.10d Fix found during the Phase 9 docs review (2026-09-25): after audio → video the previous track's file stays loaded and `isPlaying` is true, so an output **format** change (sample rate / channel count) during video made `handleEngineDidReconfigure` reschedule and play the old music under the video from the video's timestamp (and restart its progress timer → seek bar jumping). Same-format switches (LG ↔ MacBook speakers, both 48 kHz) never post the engine configuration change, which is why route tests missed it. Reproduced live by forcing the output to 44.1 kHz via CoreAudio (player node playing "Fahrenheit.m4a" under the video; user heard it + saw the seek bar jump). Fixed: reschedule/resume only when `currentMediaType == .audio`. Verified: same repro → reconfigure ran, player node not playing; user: video audio intact, no music, seek bar normal. TSan 119, only #86 fails.
