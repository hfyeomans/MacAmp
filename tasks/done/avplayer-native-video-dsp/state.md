...
>
> **Close-out (2026-09-25):** task moved to `tasks/done/avplayer-native-video-dsp/`; `_context/` advanced to S3-3 `hls-streaming-support`. Todo 10.3 (delete local `spike/avplayer-inplace-tap-dsp`) is **pending the user's decision** — the branch exists only locally and holds commit `dd53d64`, which is not in `main`. Follow-ups spawned by this task stay tracked in `tasks/_context/state.md`: multichannel video output (#88 → S4-4 `video-multichannel-output`), 16 macOS-27 deprecations (S4-1), P-6 (video→audio no auto-play), the `audioplayer-seek-extraction` trigger (fired), and the `StreamDecodePipeline.DecodeContext` concurrency-contract retrofit candidate.
>
...
| `todo.md` | ✅ COMPLETE — all phases done; only 10.3 (spike-branch deletion, user decision) and 10.8 (close-out commit) open |
| `state.md` | ✅ This file |
