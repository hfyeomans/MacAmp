...
> **Created:** 2026-05-01
> **Last revised:** 2026-09-25 (✅ MERGED — [PR #89](https://github.com/hfyeomans/MacAmp/pull/89) merged, merge commit `ae15f5c`; task moved to `tasks/done/`; post-merge close-out). Prior — 2026-09-25 (Phase 9 complete except 9.14; pushed; PR #C opened as PR #89). Prior — 2026-09-25 (Phase 8 complete; branch pushed at `5125bb3`; deployment target now macOS 27.0 via PR #87). Prior — 2026-09-05 (manual runbook started; 8.2/8.12/8.11 dispositioned; push stamp → `5fe8c3c`; LLDB + close-out sequence recorded)
> **Sprint:** S3, Wave S3-2 (architectural pivot)
> **Status:** ✅ **MERGED** — [PR #89](https://github.com/hfyeomans/MacAmp/pull/89) merged 2026-09-25 (2026-09-26 03:32 UTC), merge commit `ae15f5c`, 96 commits. Merge condition was CI green (Socket ×2, Snyk, Semgrep all passed); CodeRabbit was still processing at merge and posted no review. Pre-PR Codex 4 × P2 fixed in `e094e2e`; a later working-tree Codex P2 fixed in `bebfee5` (gitignore local session transcripts).
>
> **Close-out (2026-09-25):** task moved to `tasks/done/avplayer-native-video-dsp/`; `_context/` advanced to S3-3 `hls-streaming-support`. Todo 10.3: `spike/avplayer-inplace-tap-dsp` preserved on `origin` 2026-09-26 at the user's request (`dd53d64`, not in `main`); evaluate later whether it can be deleted. Follow-ups spawned by this task stay tracked in `tasks/_context/state.md`: multichannel video output (#88 → S4-4 `video-multichannel-output`), 16 macOS-27 deprecations (S4-1), P-6 (video→audio no auto-play), the `audioplayer-seek-extraction` trigger (fired), and the `StreamDecodePipeline.DecodeContext` concurrency-contract retrofit candidate.
>
> *Pre-merge status history:* Phase 9 ✅ complete 2026-09-25 except 9.14 (UI audit: no changes needed; docs pass + prune; pre-PR `/codex:review --base main` 4 × P2 all fixed in `e094e2e`; reconfigure-under-video fix `0f556a8`; 119 TSan tests, only pre-existing #86 fails). Phases 1-8 ✅ DONE; **Phase 8 COMPLETE 2026-09-25** — 8.1 PASS (Debug guard), 8.1b PARTIAL (`tapProcess` ≈0.4–1.0% of budget; literal 99p not producible), 8.2 NOT ABLE, 8.3/8.4/8.15 PASS, 8.5 PASS (22-min video), 8.5b-8.5e PASS, 8.6/8.7/8.10 PASS, 8.8 N/A, 8.9 PASS after the ADR-12 fix (`922d956`), 8.11 NOT ABLE AS WRITTEN, 8.12 NOT ABLE, 8.13 PASS (5.1 → stereo), 8.14 PASS, 7.9/7.10 PASS; AVKit remote-command fix `e5933ad`; 119 TSan tests, only pre-existing #86 fails. *Earlier status detail follows (as of 2026-09-05).* Automated (2026-06-27): 8.3 EQ ≤0.5 dB, 8.4 TSan **116/116**, 8.15 lifecycle (Phase 7), 8.1 CPU **Debug regression guard** (`VideoTapCPUBenchmarkTests` — DSP fits the deadline ~9× over even unoptimized; the production ≤10% figure is 8.1b MANUAL/Release-Instruments — PARTIAL 2026-09-07). Oracle 7→fixes (benchmark methodology + honesty). Hardware-manual checklist (8.5-8.14 route changes/surround/replacement + 8.5b-e live-EQ/seek/visualizer/telemetry) in **`verification.md`** — all recorded 2026-09-25. Phase 7 (Oracle 9): lifecycle tests. Phase 6 (Oracle 9): telemetry. **Phase 5 ✅ DONE (2026-05-28)**: EQ + balance state fanout (ADR-5, two canonical owners) — `EqualizerController` fans EQ (compute+`installCoefficients` + isEqOn/preamp atomics) and `AudioPlayer` fans balance to registered video-tap Contexts; sample-rate poll via `VisualizerPipeline.onPollTick`. **The audible-EQ-on-video path is now LIVE** (the deferred todo 3.17 — moving an EQ slider / balance changes video audio in real time). Oracle 9→**10/10 APPROVED**. Phase 4 (Oracle 9.6): video-tap visualizer (ADR-6). Phase 3 (Oracle 9.6): P-4 resolved (ADR-4 amendment #2 Mutex hand-off) + BiquadCascade + RBJ compute + tapProcess; ≤0.5 dB vs AVAudioUnitEQ. Steps 1-3 ✅. Plan + research locked at Oracle ≥9.8/10. Open non-blocking finding: P-6 (video→audio no auto-play; not observed either way in the 2026-09-25 8.14 run). **Manual runbook (2026-09-05 → 2026-09-25, complete)** — runbook https://claude.ai/code/artifact/1b5d48d1-5b5c-49ff-bff0-eb23beb8caf8; 8.2/8.12 dispositioned NOT ABLE, 8.11 NOT ABLE as written (PARTIAL possible); telemetry via xcodebuildmcp LLDB, Release re-signed first / Debug fallback; Xcode MCP (reconnected 2026-09-05) IS an LLDB path — RunProject(attachDebugger) + InvokeDebuggerCommand; Release telemetry read via Edit Scheme → Run → Release with no re-sign (PATH A, Claude-driven); xcodebuildmcp attach = PATH B.
...
**Tests:** 119 under TSan at the PR #89 head, only the pre-existing #86 failing (116/116 at 2026-06-27; 85/85 at the Phase 2 close — 72 baseline + 2 contract + 6 lifecycle + 5 seek-state-matrix). Engine path unchanged. *(74/74 vs 85/85 discrepancy — `todo.md` 2.37 records 74/74 for the same Phase 2 close; open item for Phase 9 pre-PR Oracle.)*
...
- **TSan crash at app launch** (`1561621`): Swift 6.4's stricter `load(as:)` alignment precondition trapped on ZIPFoundation 0.9.19's misaligned ZIP-struct read (`Data.scanValue`) when loading the default `.wsz` skin. Fixed by upgrading ZIPFoundation 0.9.19→0.9.20 (`scanValue` now uses `loadUnaligned`). TSan gate restored: 103/103 green at that point (119 tests at the PR #89 head, only pre-existing #86 failing). App was always fine in normal Debug (manual smokes passed) — TSan-only.
- Note: `project.yml` `xcodeVersion: 26.0` left as-is (cosmetic; not blocking). Deployment target stayed macOS 15.0 at the time; raised to macOS 27.0 by PR #87 (merged 2026-09-25, merged into this branch as `db3b851`).

## Branch + Wave

- **Branch:** `feat/avplayer-native-video-dsp` (cut from `main` 2026-05-01; 96 commits; ✅ merged into `main` 2026-09-25 via PR #89, merge commit `ae15f5c`)
- **Reference (paused):** `feat/video-audio-engine-routing` (44 commits, last `5af91eb`, pushed to origin)
- **Wave:** S3-2 (architectural pivot)
- **PR target:** PR #C = [PR #89](https://github.com/hfyeomans/MacAmp/pull/89) (opened 2026-09-25, ✅ merged 2026-09-25 as `ae15f5c`)
...
| `todo.md` | ✅ COMPLETE — all phases done; all close-out items done (10.3: spike branch preserved on `origin`, evaluate later; 10.8 = `0da4b97`) |
| `state.md` | ✅ This file |
...
| `verification.md` | ✅ COMPLETE — Phase 8 gate matrix + hardware-manual results (2026-06-27 → 2026-09-25); Phase 8 complete 2026-09-25 |
...
## Next steps — ✅ none (PR #89 merged 2026-09-25 as `ae15f5c`; history below)

Phases 1-7 ✅ DONE (see status banner). **Phase 8 AUTOMATED gates executed 2026-06-27** (commits `2c410a0` → `944795a` → `056c69a`): the dense-sampling CPU benchmark landed as `VideoTapCPUBenchmarkTests` — a **Debug `-Onone` DSP-core regression guard only** (p99 ≈11% / max ≈13% of the 21,333 µs deadline), plus 8.3 numerical EQ ≤0.5 dB, 8.4 TSan 116/116, and 8.15 lifecycle. **8.1b — the production CPU gate (`tapProcess` 99p ≤10% on a Release build under Instruments) — was measured 2026-09-07 and recorded PARTIAL**: `tapProcess` ≈0.4–1.0% of the callback budget, but a literal 99p is not producible (Time Profiler reports an aggregate share; `VideoTap.swift:112` samples 1-in-64 into two bucket counters). Phase 8 Oracle round 1 scored 7/10; the benchmark-methodology + honesty fixes landed in `944795a` with no post-fix re-score recorded.

The **manual/hardware checklist in `verification.md`** was run with the user 2026-09-05 → 2026-09-25 and is ✅ COMPLETE (runbook artifact https://claude.ai/code/artifact/1b5d48d1-5b5c-49ff-bff0-eb23beb8caf8, updated with all results; sprint ledger — inventory + flow diagram — at https://claude.ai/code/artifact/4556b46a-1863-4be5-8986-3b1702624a60): 8.5 PASS (22-min video), 8.5b-8.5e PASS, 8.6/8.7/8.10 PASS, 8.8 N/A (AirPlay 1 out of scope), 8.9 PASS after the ADR-12 fix, 8.13 PASS (5.1 → stereo), 8.14 PASS, 7.9/7.10 PASS. Dispositioned: **8.2 ⛔ NOT ABLE** (no Intel Mac), **8.12 ⛔ NOT ABLE** (no in-app trigger; optional), **8.11 ⛔ NOT ABLE AS WRITTEN** (no supported AAC↔SBC forcing path). Multichannel output deferred to S4-4 / #88.

**Phase 9 ✅ DONE 2026-09-25 (except 9.14):** UI audit 9.1-9.3 (no changes needed), docs 9.4-9.6c, checks 9.7-9.8, pre-PR `/codex:review --base main` 9.9 (one pass, 4 × P2) and 9.10 (all fixed in `e094e2e`), 9.12 pushed, 9.13 [PR #89](https://github.com/hfyeomans/MacAmp/pull/89) opened, 9.14 user approved and merged (`ae15f5c`). Post-merge close-out 10.1-10.7 done 2026-09-25; 10.3: spike branch preserved on `origin` 2026-09-26 (evaluate later). The sequence was (manual gates recorded in `verification.md` ✅) → Phase 9 (9.1-9.3 UI audit; 9.4-9.6 + 9.6b/9.6c mandatory docs incl. `CLAUDE.md` and `.ai-shared/macamp/project.md`; 9.7 smoke; 9.8 TSan; 9.9-9.10 pre-PR Oracle; 9.11 commit) → 9.12 push → 9.13 `gh pr create` PR #C → 9.14 human review → merge → post-merge close-out 10.1-10.8 (move the task to `tasks/done/`, delete `spike/avplayer-inplace-tap-dsp`, advance `_context` to S3-3).
