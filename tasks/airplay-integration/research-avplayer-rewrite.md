...
**Recommendation (SUPERSEDED):** ~~Stick to the current `plan.md` (Phase 1.3).~~ Phase 1.3 was attempted and failed — AVRoutePickerView cannot redirect AVAudioEngine audio on macOS. The decision is to accept system-wide AirPlay via macOS Control Center, preserving EQ, custom stream pipeline, and visualizer. See `research.md` section 8 for the full failure analysis. AVSampleBufferAudioRenderer (Path C) remains the only viable future architecture for per-app AirPlay 2 if that requirement becomes critical.