...
- **Seek scrubbing:** Progress bar in Control Center is interactive for local files; disabled for live streams
...
2. **Now Playing lifecycle requires exhaustive coverage** — Oracle caught 5 gaps across 3 review rounds. Every state transition path must call `updateNowPlayingInfo()`, including error/reconnect/auto-advance/playback-finished.
