...
> ✅ **RESOLVED 2026-09-25 — merged as [PR #89](https://github.com/hfyeomans/MacAmp/pull/89), merge commit `ae15f5c` (96 commits).** The pivot shipped; task closed out to `tasks/done/avplayer-native-video-dsp/`. This file is now a historical decision log. Next S3 work: S3-3 `hls-streaming-support`.

> **Date:** 2026-05-01 (created), 2026-09-25 (last update — RESOLVED: PR #89 merged as `ae15f5c`; header, Step-3 row, file index and decision log advanced)
> **Status:** Steps 1-3 ✅. Implementation: **Phases 1-8 ✅ DONE — Phase 8 COMPLETE 2026-09-25** (all manual/hardware gates run with the user: 8.1b PARTIAL, 8.2/8.11/8.12 NOT ABLE, 8.8 N/A, all others PASS incl. 8.9 after the ADR-12 fix; 119 TSan tests, only pre-existing #86 fails). **Phase 9 ✅ complete 2026-09-25** (UI audit — no changes; docs; pre-PR `/codex:review --base main` 4 × P2, all fixed in `e094e2e`). **✅ MERGED — PR #C = [PR #89](https://github.com/hfyeomans/MacAmp/pull/89), merge commit `ae15f5c`, 2026-09-25.** Post-merge close-out done (10.3: spike branch preserved on `origin`, evaluate later). *Earlier status detail, as of 2026-09-05:* Phase 7 (2026-06-26): lifecycle + production tests (`VideoTapLifecycleTests` 6→11 — rapid-cycle leak, injected create-failure via `_testForceTapCreateFailure` seam, attach+drop finalize, pause/resume survival, replaceCurrentItem(nil) release); **115/115 TSan**; Oracle 8→9/10 APPROVED. Phase 6 (Oracle 9): deadline-miss telemetry. Phase 5 (Oracle 10): EQ + balance fanout, audible EQ-on-video LIVE. Phases 3/4 (Oracle 9.6): BiquadCascade ≤0.5 dB + video visualizer. **Phase 8 (2026-06-27): automated gates ✅** — 8.3 EQ ≤0.5 dB, 8.4 TSan **116/116**, 8.15 lifecycle, 8.1 CPU *Debug (-Onone) regression guard* (p99 ≈11% / max ≈13% of the 21,333 µs deadline — the production ≤10% figure is gate 8.1b, Release + Instruments, **UNVERIFIED**); Oracle 7/10 → benchmark-methodology + honesty fixes. **Manual/hardware gates ⏳ READY FOR USER** (`verification.md`: 8.1b, 8.2, 8.5-8.14, 8.5b-8.5e, 7.9/7.10). **Phase 9 NEXT** (UI audit + mandatory docs + pre-PR Oracle + PR #C) — may run in parallel with the manual gates; PR #C follows them. Mid-branch: Xcode 27/Swift 6.4 migration fixes (12 warnings + ZIPFoundation 0.9.20).
> **Purpose:** Track the architectural pivot of S3-2 from engine-routing to AVPlayer-native DSP. Authoritative source for the S3-2 strategic decision log — the `_context/state.md` / `tasks_index.md` / `resume-prompt.md` files carry pivot banners pointing here, and all three were advanced in the 2026-09-25 sweeps, so their status text agrees with this file (✅ merged PR #89).
...
| Scaffold `tasks/done/avplayer-native-video-dsp/` with 6 canonical files | ✅ | All skeletons created |
...
Oracle 10/10 final after 5 rounds (7.8 → 8.9 → 9.1 → 9.5 → 10). Commits `4a80bf9` → `46bb6af`. See `tasks/done/avplayer-native-video-dsp/research.md` for the full synthesis + Evidence Ledger. Summary:
...
| Begin implementation phases (9 phases per plan §6) | ✅ Phases 1-7 ✅ DONE (Oracle-approved from Phase 2 on; Phase 1 had no Oracle round); Phase 8 ✅ COMPLETE 2026-09-25 (automated 2026-06-27, manual/hardware with the user through 2026-09-25, per `verification.md`); Phase 9 ✅ 2026-09-25; ✅ **MERGED PR #89** (`ae15f5c`, 2026-09-25) |

---

## File index

- **`tasks/done/avplayer-native-video-dsp/`** — new task folder
  - `state.md` — task-internal status (✅ MERGED PR #89, `ae15f5c`)
  ...
  - `todo.md` — ✅ populated 2026-05-02, maintained through Phase 9 — Phases 1-9 checked/dated; post-merge close-out 10.1-10.7 checked, 10.3 spike branch preserved on `origin` (evaluate later), 10.8 = close-out commit `0da4b97`
  ...
  - `verification.md` — Phase 8 gate matrix; all gates recorded (Phase 8 complete 2026-09-25)
  - `docs-update-backlog.md` — per-file docs plan feeding Phase 9 items 9.4-9.6 (executed 2026-09-25)
...
- **2026-09-07 → 2026-09-25** — Phase 8 manual/hardware gates executed (Phase A objective gates 2026-09-07; Phase B/C with the user 2026-09-25) — **Phase 8 COMPLETE 2026-09-25**; results in `verification.md`. Fixes from gate findings: `e5933ad` (AVKit remote-command handler, `updatesNowPlayingInfoCenter = false`), `55d70c2` (two regressed test gates; pre-existing failure filed as #86), `922d956` (AirPlay 2 pumping — ADR-12 preferred-format tap pinned to stereo Float32 at the source rate). PR #87 (min macOS 27) merged and merged into the branch (`db3b851`); `00d5a03` dropped the macOS 27 `#available`; `5125bb3` recorded Phase 8. Multichannel output deferred to S4-4 / #88. Branch HEAD `5125bb3`, pushed, 87 ahead of `origin/main`. **Phase 9 next.**
- **2026-09-25** — Phase 9 complete except 9.14. UI audit 9.1-9.3: no changes needed. Docs 9.4-9.6c: video DSP documented (`9d7ef92`), docs pruned 19,340 → 8,811 lines with all links verified (`4ff7364`), `ce914c1`, `d1d89ea`; review-history tags removed from code comments (`d078e57`). Fix `0f556a8`: engine reconfigure no longer resumes the previous music track under a video after an output-format change (found by the docs review, reproduced by forcing 44.1 kHz). Pre-PR `/codex:review --base main` (one pass) → 4 × P2, all fixed in `e094e2e` (video preamp bypassed when EQ is off; reconfigure will-handler skips video; `pause()` during the route-change debounce keeps the pause intent; `PreReconfigureSnapshot.wasPaused` keeps stopped tracks stopped). 119 TSan tests, only pre-existing #86 fails. Pushed (96 ahead of `origin/main`); **PR #C opened as [PR #89](https://github.com/hfyeomans/MacAmp/pull/89)**, awaiting the user's review/merge.
- **2026-09-25** — ✅ **MERGED.** [PR #89](https://github.com/hfyeomans/MacAmp/pull/89) merged by the user (merge commit `ae15f5c`, 2026-09-26 03:32 UTC; 96 commits). Merge condition was CI green (Socket ×2, Snyk, Semgrep); CodeRabbit was still processing and posted no review. A late working-tree Codex P2 was fixed pre-merge in `bebfee5` (gitignore local session transcripts). Task moved to `tasks/done/avplayer-native-video-dsp/`; this tracker marked RESOLVED. `spike/avplayer-inplace-tap-dsp` (`dd53d64`, not in `main`) preserved on `origin` 2026-09-26; evaluate later.
