...
> **Status:** Steps 1-3 ✅. Implementation: **Phases 1 + 2 + 3 + 4 + 5 ✅ DONE.** Phase 5 (2026-05-28): EQ + balance state fanout (ADR-5, two canonical owners) — `EqualizerController` fans EQ (compute+`installCoefficients` + atomics), `AudioPlayer` fans balance, to registered video-tap Contexts; sample-rate poll via `VisualizerPipeline.onPollTick`. **Audible EQ-on-video now LIVE** (deferred todo 3.17 delivered). **103/103 TSan, no races**; Oracle 9→**10/10 APPROVED**. Phase 4 (Oracle 9.6): video-tap visualizer (ADR-6). Phase 3 (Oracle 9.6): P-4 resolved + BiquadCascade + RBJ compute + tapProcess; ≤0.5 dB vs AVAudioUnitEQ. **Phase 6 NEXT** (deadline-miss telemetry). Phases 6-9 remain.
