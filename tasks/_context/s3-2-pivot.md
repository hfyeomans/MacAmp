...
> **Status:** Steps 1-3 ✅. Implementation: **Phases 1 + 2 + 3 + 4 ✅ DONE.** Phase 4 (2026-05-28): video-tap visualizer (ADR-6 dual-producer) — `videoTapVisualizerRender` → shared `VisualizerFeed`; consumer wired for video (`isVisualizerRendering` + video poll timer across spectrum/Butterchurn/oscilloscope); **98/98 TSan, no races**; Oracle 6→8→9.0→9.6 APPROVED. Phase 3 (Oracle 9.6): P-4 resolved (ADR-4 amendment #2 Mutex hand-off) + BiquadCascade + RBJ compute + tapProcess steps 2-6; ≤0.5 dB vs AVAudioUnitEQ. **Phase 5 NEXT** (EQ + balance state fanout; also delivers the deferred audible-EQ-on-video smoke). Phases 5-9 remain.
