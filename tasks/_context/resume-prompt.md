...
**PRs merged total:** 80. `feat/avplayer-native-video-dsp` is **already pushed to origin at HEAD `056c69a`** (0 ahead / 0 behind, upstream tracking set — todo 9.12 is effectively already done; its unchecked box is stale), but the branch is **unmerged and PR #C is not yet opened** (single PR at S3-2 close).
...
**Ordering assumption (unconfirmed):** the user mandated only "issues after the `.swift` rearrangement"; S4-1-before-S4-2 is our inference (its deprecation findings may change how the S4-2 issues get fixed) — confirm before scheduling. S4-1's research half touches no code and may run opportunistically earlier. Both folders are scaffolded (5 canonical files each); no research started. Full entries: `tasks/_context/tasks_index.md` § "Post-Structure-Sprint (S4)" and `tasks/_context/state.md` § "Post-Structure-Sprint (S4) — added 2026-09-05" + decision D-S4.
...
S3-1A mwvi  ✅ MERGED (PR #80, merge commit 7f3d76f, 2026-04-28)
     ...
     │       │                                                  tests; 116/116 TSan)
     │       │                                                  Phase 8 automated ✅; manual gates ⏳ user; Phase 9 NEXT (UI+docs+PR #C)
     │       │
     │       ▼
     │    S3-3 hls                               ←── PR #D
     │       │
...
You are picking up `avplayer-native-video-dsp` after Phase 8's automated gates. Steps 1+2+3 ✅ + Phases 1-7 ✅ + **Phase 8 automated gates ✅** (8.1 Debug CPU regression guard, 8.3 EQ ≤0.5 dB, 8.4 TSan 116/116, 8.15 lifecycle, 8.16 `verification.md` — commits `2c410a0`→`056c69a`, 2026-06-27). There are two live work streams. **(a) Hand the USER the hardware-manual runbook in `tasks/avplayer-native-video-dsp/verification.md`** — 8.1b Release/Instruments (the REAL `tapProcess` 99p ≤10% gate, still UNVERIFIED), 8.2 Intel, 8.5-8.14 (drift, AirPods/AirPlay/system-output/BT-codec route changes, surround, item replacement), 8.5b-8.5e (live EQ stress, seek, visualizer switch, telemetry), 7.9/7.10 signed-bundle smoke — the user runs them and records results in that file; do not tick those boxes yourself. **(b) YOU execute Phase 9** — `todo.md` items 9.1-9.11 (video-window UI audit + the `docs-update-backlog.md` doc pass incl. the mandated "Audio Mechanism Concurrency Contract" subsection), then 9.12-9.14 (push Phase 9 commits — the branch is already on origin at `056c69a` — then `gh pr create` → PR #C → human review) **only after (a) reports clean**. **Never claim the CPU gate passed:** 8.1 is a synthetic Debug (`-Onone`) DSP-core regression guard, not the production figure (per todo.md 8.1 and verification.md 8.1/8.1b).
...
2. **Read these files in order** (they describe the architecture, the contract, and the work):
   ...
   - `tasks/avplayer-native-video-dsp/todo.md` — **Phase 9 work-item checklist (9.1-9.14) is the active work.** Phases 1-7 plus Phase 8's automated items (8.1, 8.3, 8.4, 8.15, 8.16, 8.18) are `[x]`; 8.2, 8.5-8.14 and the standing rule 8.17 stay `[ ]` because they are user-owned hardware gates, and 9.12 (push) is a stale unchecked box — the branch is already pushed to origin at `056c69a`. Also `verification.md` (the user's checklist), `docs-update-backlog.md` (the Phase 9 doc work) + `placeholder.md` (P-2/P-3/P-6 open). Its header banner reads current since the 2026-09-05 sweep (Phases 1-7 ✅; Phase 8 automated ✅, manual ⏳ user; Phase 9 NEXT).
