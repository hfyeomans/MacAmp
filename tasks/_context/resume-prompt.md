...
**Tests:** **119 under TSan at `5125bb3`, only the pre-existing #86 fails** (2026-09-25). At 2026-06-27: 116/116 with TSan ON (72 baseline + 3 `VideoTapSendableContractTests` (Gate 3a/3b/3c) + **11** `VideoTapLifecycleTests` + 5 `VideoSeekStateMatrixTests` + 7 `BiquadNumericalMatchTests` + 5 `VideoTapVisualizerRenderTests` + 5 `VideoTapFanoutTests` + 7 `VideoTapTelemetryTests` + **1 `VideoTapCPUBenchmarkTests`** added by Phase 8).
**PRs merged total:** 80. `feat/avplayer-native-video-dsp` is **pushed to origin at `5125bb3` (2026-09-25)** (0 ahead / 0 behind, upstream tracking set — todo 9.12 is effectively already done; its unchecked box is stale), but the branch is **unmerged and PR #C is not yet opened** (single PR at S3-2 close).
...
- Phase 9 (NEXT for Claude): UI integration polish + final smoke + mandatory docs (execute `docs-update-backlog.md`, incl. the mandated `docs/MACAMP_ARCHITECTURE_GUIDE.md` "Audio Mechanism Concurrency Contract" subsection) + one pre-PR Codex review → PR #C
...
S3-1A mwvi  ✅ MERGED (PR #80, merge commit 7f3d76f, 2026-04-28)
     ...
     │       │                                                  tests; 116/116 TSan)
     │       │                                                  Phase 8 ✅ COMPLETE 2026-09-25; Phase 9 NEXT (UI+docs+PR #C)
     │       │
     │       ▼
     │    S3-3 hls                               ←── PR #D
     │       │
...
**After the Structure Sprint:** S4-1 `swift64-macos27-readiness` (now an adoption task), then S4-2 `github-issues-triage`; S4-3/S4-4 added 2026-09-25 — see `tasks_index.md` "Post-Structure-Sprint (S4)" and Active Work Queue item 4 above.
...
You are picking up `avplayer-native-video-dsp` after **Phase 8 completed (2026-09-25)**. Steps 1+2+3 ✅ + Phases 1-8 ✅ — every Phase 8 gate is recorded in `tasks/avplayer-native-video-dsp/verification.md` (8.1b PARTIAL, 8.2/8.11/8.12 NOT ABLE, 8.8 N/A, all others PASS; 119 TSan tests, only pre-existing #86 fails). **YOUR work is Phase 9:** `todo.md` items 9.1-9.11 (video-window UI audit + the `docs-update-backlog.md` doc pass incl. the mandated "Audio Mechanism Concurrency Contract" subsection, plus 9.6b/9.6c `CLAUDE.md` and `.ai-shared/macamp/project.md`; one pre-PR Codex review), then 9.12-9.14 (push Phase 9 commits — the branch is already on origin at `5125bb3` — then `gh pr create` → PR #C → human review). Sequence: Phase 9 (9.1-9.11) → 9.12 push → 9.13 PR #C → 9.14 human review → merge → post-merge close-out 10.1-10.8. **Never claim the CPU gate passed:** 8.1 is a synthetic Debug (`-Onone`) DSP-core regression guard, not the production figure (per todo.md 8.1 and verification.md 8.1/8.1b).

> **Phases 3-8 ✅ DONE (Phase 8 complete 2026-09-25).** Phase 3 (Oracle 9.6): BiquadCascade ≤0.5 dB. Phase 4 (Oracle 9.6): video-tap visualizer (ADR-6). Phase 5 (Oracle 10): EQ + balance fanout; audible EQ-on-video LIVE. Phase 6 (Oracle 9): deadline-miss telemetry. Phase 7 (Oracle 9): lifecycle tests (`VideoTapLifecycleTests` ×11). Phase 8 (Oracle 7 → fixes applied, no post-fix re-score): automated gates 8.1/8.3/8.4/8.15 + `verification.md` (116/116 TSan at 2026-06-27); manual gates complete 2026-09-25 (119 TSan tests, only #86 failing).
...
2. **Read these files in order** (they describe the architecture, the contract, and the work):
   ...
   - `tasks/avplayer-native-video-dsp/todo.md` — **Phase 9 work-item checklist (9.1-9.14) is the active work.** Phases 1-8 are `[x]` except 8.11 (NOT ABLE AS WRITTEN) and the standing rule 8.17, and 9.12 (push) is a stale unchecked box — the branch is already pushed to origin at `5125bb3` (2026-09-25). Also `verification.md` (Phase 8 record), `docs-update-backlog.md` (the Phase 9 doc work) + `placeholder.md` (P-2/P-3/P-6 open). Its header banner reads current since the 2026-09-25 sweep (Phases 1-8 ✅; Phase 9 NEXT).
...
3. **Re-read at HEAD** the files/areas Phase 9 (UI polish + mandatory docs) touches:
   ...
   - `tasks/avplayer-native-video-dsp/verification.md` — the Phase 8 record (complete 2026-09-25); source for the Phase 9 docs' verification claims. Engine path stays UNTOUCHED.
...
6. **Then execute Phase 9** (UI integration polish + mandatory docs) per `todo.md` items 9.1-9.11 + plan.md §6 Phase 9. Essentials:
   ...
   - 9.7-9.8: end-to-end smoke (video + EQ drag + balance + visualizer-mode switch) and a full TSan-on suite run (expect 119 tests, only the pre-existing #86 failing).
...
### Phase 9 — the active Claude work (Phase 8 complete 2026-09-25)
