...
- **Xcode MCP conclusion (revised same day):** Xcode's own MCP server (`mcp__xcode__*`) failed to connect directly (`CONNECTION_CLOSED`), but it IS reachable through the xcodebuildmcp bridge (`xcodebuildmcp xcode-ide list-tools`, run from a user terminal) and exposes **47 tools** — `GetConsoleOutput` (stdout/stderr/OSLog of an Xcode-launched app with regex + severity filters) was confirmed; the bridge timed out (`BRIDGE_LIST_TIMEOUT`) before the full list could be checked for launch/breakpoint/expression tools. **Open:** the user should enumerate the list; if debugger tools exist, an Xcode *Run* of the Release scheme attaches without the get-task-allow re-sign. Until confirmed, `xcodebuildmcp debugging` is the LLDB path; the previously documented surface (build logs, navigator issues, previews, snippet execution, window/tab context) is a subset.
