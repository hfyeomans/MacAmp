...
> ⚠️ **S3-2 ARCHITECTURAL PIVOT — IMPLEMENTATION IN PROGRESS (2026-05-02).** `feat/video-audio-engine-routing` is **PAUSED-AS-REFERENCE** (preserved at `5af91eb`, pushed to origin). S3-2 re-attempted as **`avplayer-native-video-dsp`** on branch `feat/avplayer-native-video-dsp`. **Steps 1+2+3 ✅ + Phases 1 + 2 + 3 + 4 + 5 + 6 ✅ done. Phase 7 (lifecycle + production tests) NEXT.** See `tasks/_context/s3-2-pivot.md` for the strategic decision log + step-by-step status — that file is authoritative.

**Last update:** 2026-06-26 (Phase 6 ✅ **DONE** — deadline-miss telemetry on the video tap. RT-safe `Atomic<UInt64>` counters (`budgetOverrunCount`/`deadlineRiskCount`/`lastDeadlineRiskHostTime`) sampled every 64th `tapProcess` callback via a cached Mach timebase (prewarmed off the render thread in `buildAudioMix`); `VideoTapDiagnostics` main-thread snapshot; pure `recordProcessingDeadline` eval as the test seam. **NO render-thread logging** (RT-safety). Also fixed a real `tapPrepare` store-ordering bug — `pendingSampleRate`/`isActive` now publish BEFORE the `.releasing` store of `processingFormatTag` (the render acquire-loads the tag as its gate). **110/110 tests with TSan, no races.** Oracle 8→**9/10 APPROVED**. Commits `82365c7`→`3fd157c`. **Phase 7 NEXT** (lifecycle + production tests). Also mid-branch: Xcode 26→27/Swift 6.4 migration — fixed 12 warnings (`786b3c2`) + ZIPFoundation 0.9.19→0.9.20 for a TSan launch crash (`1561621`). Open non-blocking: P-6.)
**Main HEAD:** `9cca40a` (main has not advanced during S3-2 work).
**`feat/avplayer-native-video-dsp` HEAD:** `3fd157c` (run `git log -1 --oneline` to confirm — doc commits may have advanced it). **~66 commits ahead of main** (run `git rev-list --count main..HEAD` for the live number). Commit arc: Phase 1 `146a8b4` → Phase 2 `ac7e0d5`→`7d3367c` (Oracle 9.0) → Phase 3 `37f9edc`→`ddd0431` (Oracle 7→9.6) → Phase 4 `92d0079`→`ea95f10` (Oracle 6→9.6) → Phase 5 `e1f8a4e`→`252d3bc` (Oracle 9→10) → Xcode-27 toolchain fixes `786b3c2`/`1561621` (warnings + ZIPFoundation) → **Phase 6 `82365c7`→`3fd157c` (deadline-miss telemetry + Oracle 8→9).**
...
**Tests:** 110/110 with TSan ON (72 baseline + 3 `VideoTapSendableContractTests` (Gate 3a/3b/3c) + 6 `VideoTapLifecycleTests` + 5 `VideoSeekStateMatrixTests` + 7 `BiquadNumericalMatchTests` + 5 `VideoTapVisualizerRenderTests` + 5 `VideoTapFanoutTests` + 7 `VideoTapTelemetryTests`). Target by S3-2 close: ~116+ (Phase 7 additional lifecycle ~6 tests).
...
**Status:** Steps 1+2+3 ✅ + Phases 1 + 2 + 3 + 4 + 5 + 6 ✅ done. **Phase 7 (lifecycle + production tests) NEXT** — see `tasks/avplayer-native-video-dsp/todo.md` for the per-phase work breakdown.
...
- Phase 7 (NEXT): lifecycle + production tests (TSan, signed-bundle smoke) — tap create/attach/detach stress cycles, `passRetained`↔`tapFinalize` balance accounting, injected tap-create-failure path
...
S3-1A mwvi  ✅ MERGED (PR #80, merge commit 7f3d76f, 2026-04-28)
     ...
     │       │                                                  Step 1+2+3 ✅; Phases 1-6 ✅
     │       │                                                  (P3 EQ ≤0.5dB; P4 visualizer; P5 fanout —
     │       │                                                  audible EQ live; P6 telemetry; 110/110 TSan)
     │       │                                                  Phase 7 NEXT (lifecycle tests); 3 phases remain
     │       │
     │       ▼
     │    S3-3 hls                               ←── PR #D
     │       │
...
You are picking up `avplayer-native-video-dsp` mid-implementation. Steps 1+2+3 ✅ + Phases 1 + 2 + 3 + 4 + 5 + 6 ✅ are done. **Phase 7 (lifecycle + production tests, per plan.md §6 Phase 7) is the active work** — see `tasks/avplayer-native-video-dsp/todo.md` Phase 7 items. Add tap create/attach/detach stress cycles (10 cycles in 1 s → no leak), `Unmanaged.passRetained`↔`tapFinalize` balance accounting via `_test*` seams, the injected tap-create-failure path (`_testForceTapCreateFailure`), and a signed-bundle video smoke. Builds on the existing `VideoTapLifecycleTests`.

> **Phases 3-6 ✅ DONE.** Phase 3 (Oracle 9.6): P-4 resolved (ADR-4 amendment #2 Mutex hand-off) + `BiquadCascade` + RBJ `compute` + `tapProcess` steps 2-6; ≤0.5 dB vs `AVAudioUnitEQ`. Phase 4 (Oracle 9.6): video-tap visualizer (ADR-6). Phase 5 (Oracle 10/10): EQ + balance fanout; audible EQ-on-video LIVE. Phase 6 (Oracle 9): deadline-miss telemetry (RT-safe counters, `VideoTapDiagnostics`). 110/110 TSan. The Phase 3 detail below is retained as historical reference.
...
3. **Re-read at HEAD** the files Phase 7 (lifecycle + production tests) will touch (line numbers drift):
   - `Tests/MacAmpTests/VideoTapLifecycleTests.swift` — the existing 6 lifecycle tests; Phase 7 ADDS the stress cycles (10 create/attach/play/replace in 1 s → no leak), the injected create-failure path, and pause/resume/seek lifecycle. Build on these patterns.
   - `MacAmpApp/Audio/VideoDSP/VideoTap.swift` + `VideoTapContext.swift` — may need `_test*` seams (e.g. `_testForceTapCreateFailure`) for the failure-injection test; the `Unmanaged.passRetained`↔`tapFinalize` balance is the thing under test (ADR-10).
   - `MacAmpApp/Audio/AudioPlayer.swift` — `startVideoLoad` / `pauseAndDetachVideoTapIfNeeded` are the orchestration the lifecycle tests exercise.
   - Phase 7 also covers the signed-bundle smoke (build + sign + launch + play video). Engine path stays UNTOUCHED.
...
Phase 8 executes the 15-gate verification matrix (reads the Phase 6 deadline-miss counters as the CPU gate — **note: Phase 8 needs DENSE per-callback sampling, not the advisory 1/64 Phase 6 uses**). Phase 9 is UI integration polish + mandatory docs (`docs/MACAMP_ARCHITECTURE_GUIDE.md` "Audio Mechanism Concurrency Contract" subsection — the Phase 2-6 architecture deltas to fold in are catalogued in `tasks/avplayer-native-video-dsp/docs-update-backlog.md` — detailed per-doc, line-referenced, from a 5-agent full-file audit; high-level summary in `state.md`) + pre-PR Oracle review + `gh pr create`.
