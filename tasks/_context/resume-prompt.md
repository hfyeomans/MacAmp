...
**Tests:** 115/115 with TSan ON (72 baseline + 3 `VideoTapSendableContractTests` (Gate 3a/3b/3c) + **11** `VideoTapLifecycleTests` + 5 `VideoSeekStateMatrixTests` + 7 `BiquadNumericalMatchTests` + 5 `VideoTapVisualizerRenderTests` + 5 `VideoTapFanoutTests` + 7 `VideoTapTelemetryTests`). Phase 8 may add a few benchmark tests.
...
**Status:** Steps 1+2+3 ✅ + Phases 1 + 2 + 3 + 4 + 5 + 6 + 7 ✅ done. **Phase 8 (15-gate verification matrix) NEXT** — see `tasks/avplayer-native-video-dsp/todo.md` for the per-phase work breakdown.
...
- Phase 8 (NEXT): verification matrix execution (15 gates: Static / Dynamic / Lifecycle) — CPU benchmark (needs DENSE per-callback sampling, not Phase 6's advisory 1/64), numerical EQ ≤0.5 dB re-run, TSan; AirPods/AirPlay route changes during video (HARDWARE-MANUAL), surround, long-playback drift
...
S3-1A mwvi  ✅ MERGED (PR #80, merge commit 7f3d76f, 2026-04-28)
     ...
     │       │                                                  tests; 115/115 TSan)
     │       │                                                  Phase 8 NEXT (verification matrix); 2 phases remain
     │       │
     │       ▼
     │    S3-3 hls                               ←── PR #D
     │       │
...
You are picking up `avplayer-native-video-dsp` mid-implementation. Steps 1+2+3 ✅ + Phases 1 + 2 + 3 + 4 + 5 + 6 + 7 ✅ are done. **Phase 8 (15-gate verification matrix, per plan.md §6 Phase 8) is the active work** — see `tasks/avplayer-native-video-dsp/todo.md` Phase 8 items (8.1-8.18). Static gates: CPU benchmark of `tapProcess` (**needs DENSE per-callback sampling — Phase 6's 1/64 counters are advisory; add a benchmark mode or offline measurement**), numerical EQ ≤0.5 dB re-run (`BiquadNumericalMatchTests`), TSan full suite. Dynamic gates: AirPods/AirPlay route changes during video, system output switch, BT codec switch, long-playback A/V drift, surround, item replacement — **many are HARDWARE-MANUAL (route changes need real devices)**. Document each gate's pass/fail + commit SHA in `state.md` (or a new `verification.md`).

> **Phases 3-7 ✅ DONE.** Phase 3 (Oracle 9.6): BiquadCascade ≤0.5 dB. Phase 4 (Oracle 9.6): video-tap visualizer (ADR-6). Phase 5 (Oracle 10): EQ + balance fanout; audible EQ-on-video LIVE. Phase 6 (Oracle 9): deadline-miss telemetry. Phase 7 (Oracle 9): lifecycle tests (`VideoTapLifecycleTests` ×11). 115/115 TSan. The Phase 3 detail below is retained as historical reference.
...
2. **Read these files in order** (they describe the architecture, the contract, and the work):
   ...
   - `tasks/avplayer-native-video-dsp/todo.md` — **Phase 8 work-item checklist (8.1-8.18) is the active work.** Phases 1-7 items are all `[x]` (manual/hardware smokes flagged READY FOR USER). Also `docs-update-backlog.md` (Phase 9 doc work) + `placeholder.md` (P-2/P-3/P-6 open).
...
3. **Re-read at HEAD** the files/areas Phase 8 (verification matrix) touches:
   - `MacAmpApp/Audio/VideoDSP/VideoTapContext.swift` — the Phase 6 telemetry counters (`diagnosticSnapshot`) are the CPU-gate readout; Phase 8 likely adds a denser benchmark seam (every-callback timing) since the production 1/64 sampling is advisory.
   - `Tests/MacAmpTests/BiquadNumericalMatchTests.swift` — re-run for the ≤0.5 dB gate (8.3).
   - `docs/RELEASE_BUILD_GUIDE.md` — for the signed-bundle / CPU-profile runs.
   - Most Dynamic gates (route changes, BT codec, system output) are HARDWARE-MANUAL — Phase 8 is largely a documentation-of-results exercise, not new code. Engine path stays UNTOUCHED.
