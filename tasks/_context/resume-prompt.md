...
> ⚠️ **S3-2 ARCHITECTURAL PIVOT — IMPLEMENTATION IN PROGRESS (2026-05-02).** `feat/video-audio-engine-routing` is **PAUSED-AS-REFERENCE** (preserved at `5af91eb`, pushed to origin). S3-2 re-attempted as **`avplayer-native-video-dsp`** on branch `feat/avplayer-native-video-dsp`. **Steps 1+2+3 ✅ + Phases 1 + 2 + 3 + 4 ✅ done. Phase 5 (EQ + balance state fanout) NEXT.** See `tasks/_context/s3-2-pivot.md` for the strategic decision log + step-by-step status — that file is authoritative.

**Last update:** 2026-05-28 (Phase 4 ✅ **DONE** — video-tap visualizer, ADR-6 dual-producer. `videoTapVisualizerRender` (AudioBufferList → shared `VisualizerFeed`; mono downmix non-interleaved+interleaved, 20-bar RMS + Goertzel duplicated per ADR-6, FFT shared); `VideoTapContext` gained `feed`(injected)/`scratch`(owned); `tapProcess` step 7 publishes the post-DSP buffer. **Consumer wired for video** (Oracle round-1 blockers): `VisualizerPipeline.start/stopVideoVisualization` drives the 30 Hz poll timer for video; `AudioPlayer.isVisualizerRendering` (engine OR video) ungates `getFrequencyData`/`snapshotButterchurnFrame`/`VisualizerView`/oscilloscope; lifecycle hooked across video start / video→audio / stop / completion / repeat-one. **98/98 tests with TSan, no races.** Oracle arc 6→8→9.0→**9.6/10 APPROVED**. Commits `92d0079`→`2884033`→`d475374`→`1634dbd`. Phase 3 (Oracle 9.6): P-4 resolved (ADR-4 amendment #2 Mutex hand-off) + BiquadCascade + RBJ compute + tapProcess steps 2-6, ≤0.5 dB vs AVAudioUnitEQ. **Phase 5 NEXT** (EQ + balance state fanout — also delivers the deferred audible-EQ-on-video smoke, todo 3.17). Open non-blocking: P-6.)
**Main HEAD:** `9cca40a` (main has not advanced during S3-2 work).
**`feat/avplayer-native-video-dsp` HEAD:** `ddd0431` (run `git log -1 --oneline` to confirm — doc commits may have advanced it). **~48 commits ahead of main** (run `git rev-list --count main..HEAD` for the live number). Commit arc: 13 cherry-picked Phase-1 commits (ending `2aa2f18`) + pivot cleanup + scaffold → Step-2 research `4a80bf9`→`46bb6af` (Oracle 10/10) → Step-3 plan `1ae8e80`→`fdce0ed` (Oracle 9.8/10) → Phase 1 `146a8b4` → Phase 2 `ac7e0d5`→`7d3367c` (Option C, Oracle 9.0) → Phase 2 close (todo 2.40 leak check + P-6 + instruments doc) → **Phase 3 `37f9edc`→`ddd0431` (P-4 resolution + BiquadCascade + tapProcess + Oracle 7→9.6 remediation).**
...
**Tests:** 98/98 with TSan ON (72 baseline + 3 `VideoTapSendableContractTests` (Gate 3a/3b/3c) + 6 `VideoTapLifecycleTests` + 5 `VideoSeekStateMatrixTests` + 7 `BiquadNumericalMatchTests` + 5 `VideoTapVisualizerRenderTests`). Target by S3-2 close: ~110+ (Phase 7 additional lifecycle ~6 tests).
...
**Status:** Steps 1+2+3 ✅ + Phases 1 + 2 + 3 + 4 ✅ done. **Phase 5 (EQ + balance state fanout) NEXT** — see `tasks/avplayer-native-video-dsp/todo.md` for the per-phase work breakdown.
...
**Phase 4 (visualizer DSP, ADR-6 dual-producer) ✅ DONE 2026-05-28.** `videoTapVisualizerRender` (AudioBufferList → shared `VisualizerFeed`; RMS/Goertzel duplicated per ADR-6, FFT shared); `VideoTapContext` gained `feed`(injected)/`scratch`(owned); `tapProcess` step 7. Consumer wired for video — `VisualizerPipeline.start/stopVideoVisualization` poll timer + `AudioPlayer.isVisualizerRendering` ungating across spectrum/Butterchurn/oscilloscope; lifecycle across video start/switch/stop/completion/repeat-one. 98/98 TSan; Oracle 6→8→9.0→**9.6 APPROVED**. Commits `92d0079`→`2884033`→`d475374`→`1634dbd`.

**Remaining phases (per plan.md §6):**
- Phase 5 (NEXT): EQ + balance state fanout (parallel from `EqualizerController` + `AudioPlayer`) — **also delivers the audible-EQ-on-video smoke deferred from Phase 3 (todo 3.17)**; writes coefficients via `Context.installCoefficients` (the Mutex) + the `isEqOn`/`preamp`/`balance` atomics — never `.cascade` (render-confined, guarded by the `cascadeIsRenderConfined` contract test)
...
S3-1A mwvi  ✅ MERGED (PR #80, merge commit 7f3d76f, 2026-04-28)
     ...
     │       │                                                  Step 1+2+3 ✅; Phases 1-4 ✅
     │       │                                                  (P3: EQ DSP ≤0.5dB; P4: video visualizer,
     │       │                                                  Oracle 9.6; 98/98 TSan)
     │       │                                                  Phase 5 NEXT (EQ/balance fanout); 5 phases remain
     │       │
     │       ▼
     │    S3-3 hls                               ←── PR #D
     │       │
...
You are picking up `avplayer-native-video-dsp` mid-implementation. Steps 1+2+3 ✅ + Phases 1 + 2 + 3 + 4 ✅ are done. **Phase 5 (EQ + balance state fanout, per plan.md §6 Phase 5 + ADR-5) is the active work** — see `tasks/avplayer-native-video-dsp/todo.md` Phase 5 items. Fan out `EqualizerController`/`AudioPlayer` state to the registered video-tap Context(s): write coefficients via `Context.installCoefficients` (the Mutex; `EqualizerController.equalizerState` projection is ready) + the `isEqOn`/`preamp`/`balance` atomics, on each EQ/preamp/balance/preset/sample-rate change. **NEVER touch `.cascade`** (render-confined; the `cascadeIsRenderConfined` test fails the build if you do). Phase 5 also delivers the deferred audible-EQ-on-video smoke (todo 3.17) — the first phase where moving an EQ slider changes video audio.

> **Phases 3 + 4 ✅ DONE (2026-05-28).** Phase 3 (Oracle 9.6): P-4 resolved (ADR-4 amendment #2 Mutex hand-off) + `BiquadCascade` + RBJ `compute` + `tapProcess` steps 2-6; ≤0.5 dB vs `AVAudioUnitEQ`. Phase 4 (Oracle 9.6): video-tap visualizer (ADR-6 dual-producer) — `videoTapVisualizerRender` → shared `VisualizerFeed`; consumer wired for video (`isVisualizerRendering` + video poll timer). 98/98 TSan. The Phase 3 detail below is retained as historical reference.
...
3. **Re-read at HEAD** the files Phase 5 (EQ + balance fanout) will touch (line numbers drift):
   - `MacAmpApp/Audio/EqualizerController.swift` — owns EQ state; `equalizerState` projection is ready. Phase 5 adds the video-tap Context registry + fanout: on each EQ/preamp/balance/preset change, compute `BiquadCoefficientSet.compute(for:sampleRate:)` and `installCoefficients` it on each registered Context + write the `isEqOn`/`preamp` atomics.
   - `MacAmpApp/Audio/AudioPlayer.swift` — `startVideoLoad`'s `audioMixBuilder` registers the new Context with the fanout registry; `balance` setter writes the Context's `balance` atomic; `pauseAndDetachVideoTapIfNeeded`/`invalidateInFlightVideoLoad`/stop unregister.
   - `MacAmpApp/Audio/VideoDSP/VideoTapContext.swift` — final shape: `coefficients: Mutex<…>` (write via `installCoefficients`), `cascade`/`feed`/`scratch` (NEVER touch from main — `.cascade` guarded by a test), `isEqOn`/`preampLinearGainBits`/`balance`/`pendingSampleRate` atomics. Sample-rate recompute polls `pendingSampleRate` (ADR-5 §"Sample-rate handling").
   - `MacAmpApp/Audio/VideoDSP/BiquadCoefficientSet.swift` — `compute(for:sampleRate:)` is the fanout's coefficient source.
   - Engine path (`makeTapHandler`, audio EQ) stays UNTOUCHED; Phase 5 only adds the video fanout.
...
- **Audible EQ-on-video smoke (todo 3.17) DEFERRED to Phase 5** — the tap's coefficients are never populated until the Phase 5 fanout wires `EqualizerController`/`AudioPlayer` → `installCoefficients` + atomics. Phase 3's DSP is verified by automated numerical-match tests, not by ear yet.

### After Phase 5

Phase 6 adds deadline-miss telemetry. Phase 7 is the lifecycle test suite + signed-bundle smoke. Phase 8 executes the 15-gate verification matrix. Phase 9 is UI integration polish + mandatory docs (`docs/MACAMP_ARCHITECTURE_GUIDE.md` "Audio Mechanism Concurrency Contract" subsection — the Phase 2-4 architecture deltas to fold in are catalogued in `tasks/avplayer-native-video-dsp/state.md` "Architecture / flow changes for a later docs/ update") + pre-PR Oracle review + `gh pr create`.
