...
- **2026-09-25 (close-out) — PR #89 MERGED.** The user merged [PR #89](https://github.com/hfyeomans/MacAmp/pull/89) (merge commit `ae15f5c`, 2026-09-26 03:32 UTC, 96 commits) once CI was green (Socket ×2, Snyk, Semgrep); CodeRabbit was still processing and posted no review. A late working-tree Codex P2 was fixed pre-merge in `bebfee5` (gitignore `macamp-save-*.md` / `macamp-converstion-*.md`). Close-out 10.1-10.7 done: task moved to `tasks/done/avplayer-native-video-dsp/`, `_context/` advanced to S3-3, `s3-2-pivot.md` RESOLVED. **10.3 pending the user's decision** — `spike/avplayer-inplace-tap-dsp` is local-only (no remote) and holds `dd53d64`, which is not in `main`; deleting it loses that commit.
...
**`spike/avplayer-inplace-tap-dsp` HEAD (throwaway, local-only, no remote):** `dd53d64` — Phase 0 spike, **not in `main`**. Deletion (todo 10.3) is pending the user's decision — deleting the branch loses the commit.
...
**Just closed:** S3-2 `avplayer-native-video-dsp` — see `tasks/done/avplayer-native-video-dsp/` and "Current State" above. Pending user decision: delete the local-only `spike/avplayer-inplace-tap-dsp` (holds `dd53d64`, not in `main`)?
...
**Spike policy (default — do NOT deviate without explicit reason):** each Phase 0 spike runs at its parent task's pickup time on a throwaway branch, findings written to that task's `research.md`, branch deleted. S3-2's spike (`spike/avplayer-inplace-tap-dsp`) ran 2026-05-01 and is still **retained locally** after the S3-2 merge — its commit `dd53d64` is not in `main` and the branch has no remote, so deletion (todo 10.3) awaits the user's decision.
...
**Before anything else, ask the user about the pending S3-2 close-out item 10.3:** delete the local-only `spike/avplayer-inplace-tap-dsp` branch? It holds `dd53d64`, which is not in `main` and has no remote copy — deleting it loses the commit. Do not delete it without a yes.
