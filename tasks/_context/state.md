...
| Current phase | S3 — S3-1 ✅ merged; S3-2 `avplayer-native-video-dsp` 🔍 IN REVIEW (Phases 1-9 ✅; **[PR #89](https://github.com/hfyeomans/MacAmp/pull/89) open, awaiting the user's review/merge**; then close-out 10.1-10.8); S3-3 / S3-4 queued behind the S3-2 merge |
| PRs merged | Highest merged: **#87** `chore/min-macos-27` (D-TARGET27, 2026-09-25) — `origin/main` HEAD is `c79c2ca`, merged into the S3-2 branch as `db3b851`. S3-2 PR #C = [PR #89](https://github.com/hfyeomans/MacAmp/pull/89), open (not merged). |
| Architecture principles | `tasks/_context/principles.md` |

---

## Current Phase: v1.3 RELEASED — S3 In Progress (S3-2 PR #89 in review)
...
> **Status (2026-09-25):** Wave S3-1 ✅ **COMPLETE** — S3-1A `mainwindow-visualizer-isolation` merged PR #80 (2026-04-28); S3-1B `stream-pause-tail` merged PR #82 (2026-04-30, merge commit `b60fd57`). Post-S3-1A follow-up `timer-runloop-mode-audit` merged PR #81 (2026-04-29). S3-2 pivoted 2026-05-01 from `video-audio-engine-routing` (PAUSED-AS-REFERENCE at `5af91eb`) to **`avplayer-native-video-dsp`**, branch `feat/avplayer-native-video-dsp`, **96 commits ahead of `origin/main` (`c79c2ca`), pushed to origin (local == origin); unmerged; PR #C = [PR #89](https://github.com/hfyeomans/MacAmp/pull/89), opened 2026-09-25, awaiting the user's review/merge**. Steps 1-3 ✅ (research 10/10, plan 9.8/10); implementation **Phases 1-8 ✅ DONE** — **Phase 8 COMPLETE 2026-09-25** per `tasks/avplayer-native-video-dsp/verification.md`: 8.1 PASS (Debug guard), 8.1b PARTIAL (Time Profiler: `tapProcess` ≈0.4–1.0% of budget; literal 99p not producible), 8.2 NOT ABLE (no Intel Mac), 8.3/8.4/8.15 PASS, 8.5 PASS (22-min video), 8.5b-8.5e PASS, 8.6/8.7/8.10 PASS, 8.8 N/A, 8.9 PASS after the ADR-12 fix, 8.11 NOT ABLE AS WRITTEN, 8.12 NOT ABLE, 8.13 PASS (5.1 → stereo), 8.14 PASS, 7.9/7.10 PASS. Two fixes landed from gate findings (`e5933ad` AVKit remote commands, `922d956` AirPlay 2 pumping). **Phase 9 ✅ complete 2026-09-25 except 9.14** — UI audit (no changes), docs + prune, reconfigure-under-video fix `0f556a8`, pre-PR `/codex:review --base main` 4 × P2 fixed in `e094e2e`. **Now:** PR #89 awaiting the user's review/merge, then close-out 10.1-10.8. S3-3 / S3-4 still queued behind the S3-2 merge.
...
| S3-2 (pivot) | sequential | `avplayer-native-video-dsp` | `feat/avplayer-native-video-dsp` | **#89** (open) | S3-1 merged ✅ + Phase 1 cherry-pick ✅ | `spike/avplayer-inplace-tap-dsp` ✅ done (branch still undeleted — close-out 10.3) | 🔍 **IN REVIEW** — [PR #89](https://github.com/hfyeomans/MacAmp/pull/89) opened 2026-09-25, awaiting the user's review/merge; 96 commits ahead of `origin/main`, pushed. Steps 1-3 ✅; **Phases 1-7 ✅** Oracle-approved; **Phase 8 ✅ COMPLETE 2026-09-25** (all manual gates run with the user; 8.1b PARTIAL, 8.2/8.11/8.12 NOT ABLE, 8.8 N/A, rest PASS — `tasks/avplayer-native-video-dsp/verification.md`; 119 TSan tests, only pre-existing #86 fails); **Phase 9 ✅ 2026-09-25** (except 9.14 human review). See `_context/s3-2-pivot.md`. |
...
| P-6 | 2026-05-28 | internal | Video→audio transition does not auto-play | Folded in from `tasks/avplayer-native-video-dsp/placeholder.md` (non-blocking; not observed either way in the 2026-09-25 8.14 run). Also tracked in the "Post-S3-2 `avplayer-native-video-dsp` Findings" section above |
