...
> **Date:** 2026-02-21
> **Updated:** 2026-04-30 (S3-2 Phase 0 + 1 + 2 + 3 + 5 ✅ **all complete**. Phase 5 ships a 250 ms `@MainActor` watchdog observing `VideoAudioTap` for stalls (>1 s host-time gap) and process-side errors (`fallbackRequested` flag set by `MTAudioProcessingTapGetSourceAudio` failure or `AudioConverterFillComplexBuffer` fault); on trigger `engageVideoTapFallback()` demotes the session from the engine bridge to direct AVPlayer audio so the user keeps hearing sound. Pause→resume baseline reset (`max(last, resumeBaselineHost)`) prevents stale-callback false positives. Co-fixed: VideoAudioTap process-side fallback flagging + `lastCallbackHostTime` only-after-ring-write invariant, plus `startVideoTrack` guards `engine.isVideoBridgeActive` after activation so silent-mute-0 video can't slip through on engine refusal. 1 commit (`adf3fa4`); Oracle gpt-5.5 xhigh **9.2/10** pass 2 (gate clear ≥9.0); 94/94 tests pass with TSan (added 4: engageRestoresAVPlayerVolume, engageIsIdempotent, watchdogEngagesOnFallbackRequested, playTrackResetsFallbackFlag). **Phase 6 (capability flag surface per plan §11) is next; Phase 4 is no-op per Phase 0 Path NONE.**)
> **Previous:** 2026-04-30 (S3-2 Phase 0 + 1 + 2 + 3 ✅ — Phase 3 wires `videoSourceNode` parallel to `streamSourceNode`; 9 commits closed at **9.5/10** after regression-fix arc; manual video verified.)
...
> **Status (2026-04-30):** Wave S3-1 ✅ **COMPLETE** — S3-1A `mainwindow-visualizer-isolation` merged PR #80 (2026-04-28); S3-1B `stream-pause-tail` merged PR #82 (2026-04-30, merge commit `b60fd57`). Post-S3-1A follow-up `timer-runloop-mode-audit` merged PR #81 (2026-04-29). S3-2 `video-audio-engine-routing` Phase 0 + 1 + 2 + 3 + 5 all ✅ complete (2026-04-30) — Path NONE confirmed empirically; engine config observer ships engine-recovery-on-output-route-change; MTAudioProcessingTap wrapper ships at 9.3/10; engine source node + AudioPlayer wiring ships at **9.5/10 final** after a 3-commit regression-fix arc post real-video manual test (impl Oracle: 8.4→9.2→9.4; regression-fix Oracle: 7→8→9.5); tap-failure watchdog + AVPlayer fallback ships at **9.2/10** Oracle gpt-5.5 pass 2 with co-fixes for VideoAudioTap process-side fallback flagging and activateVideoBridge internal-failure handling. 32 commits on `feat/video-audio-engine-routing`; 94/94 tests pass with TSan; manual video verified (display, single audio, EQ, spectrum). Milkdrop intentionally deferred to Phase 6 per plan §11.3. **Phase 6 (capability flag surface per plan §11) is next; Phase 4 is no-op per Phase 0 Path NONE.** S3-3 / S3-4 still queued behind S3-2 PR.
...
| S3-2 | sequential | `video-audio-engine-routing` | `feat/video-audio-engine-routing` | C | S3-1 merged ✅ + Phase 0 ✅ + Phase 1 ✅ + Phase 2 ✅ + Phase 3 ✅ + Phase 5 ✅ | `spike/vaer-av-drift-measurement` ✅ deleted | 🔧 **IN PROGRESS** — Phase 0/1/2/3/5 done; Phase 6 (capability flag surface per plan §11) next; Phase 4 no-op |
...
| `video-audio-engine-routing` | ✅ existing | ✅ | ✅ | **9.4/10** (plan); **9.5/10** Phase 3; **9.2/10** Phase 5 (Phase 1: 9.5; Phase 2: 9.3; Phase 3: 9.5 final after regression-fix arc; Phase 5: 9.2 pass 2) | 3 plan + 1+3+5 impl |
