...
| .swift files | ~122 |
| Tests | 119 under TSan at branch HEAD `5125bb3`; only the pre-existing #86 fails |
| Current phase | S3 — S3-1 ✅ merged; S3-2 `avplayer-native-video-dsp` 🔧 IN PROGRESS (**Phase 8 ✅ complete 2026-09-25** — all manual gates run; **Phase 9 next**; pushed to origin, no PR yet); S3-3 / S3-4 queued |
| PRs merged | Highest merged: **#87** `chore/min-macos-27` (D-TARGET27, 2026-09-25) — `origin/main` HEAD is `c79c2ca`, merged into the S3-2 branch as `db3b851`. S3-2 PR (#C) not yet opened. |
| Architecture principles | `tasks/_context/principles.md` |

---

## Current Phase: v1.3 RELEASED — S3 In Progress (S3-2 Phase 9 next)
...
> **Status (2026-09-25):** Wave S3-1 ✅ **COMPLETE** — S3-1A `mainwindow-visualizer-isolation` merged PR #80 (2026-04-28); S3-1B `stream-pause-tail` merged PR #82 (2026-04-30, merge commit `b60fd57`). Post-S3-1A follow-up `timer-runloop-mode-audit` merged PR #81 (2026-04-29). S3-2 pivoted 2026-05-01 from `video-audio-engine-routing` (PAUSED-AS-REFERENCE at `5af91eb`) to **`avplayer-native-video-dsp`**, branch `feat/avplayer-native-video-dsp`, **HEAD `5125bb3`, 87 commits ahead of `origin/main` (`c79c2ca`), pushed to origin (local == origin); unmerged; PR #C not yet opened**. Steps 1-3 ✅ (research 10/10, plan 9.8/10); implementation **Phases 1-8 ✅ DONE** — **Phase 8 COMPLETE 2026-09-25** per `tasks/avplayer-native-video-dsp/verification.md`: 8.1 PASS (Debug guard), 8.1b PARTIAL (Time Profiler: `tapProcess` ≈0.4–1.0% of budget; literal 99p not producible), 8.2 NOT ABLE (no Intel Mac), 8.3/8.4/8.15 PASS, 8.5 PASS (22-min video), 8.5b-8.5e PASS, 8.6/8.7/8.10 PASS, 8.8 N/A, 8.9 PASS after the ADR-12 fix, 8.11 NOT ABLE AS WRITTEN, 8.12 NOT ABLE, 8.13 PASS (5.1 → stereo), 8.14 PASS, 7.9/7.10 PASS. Two fixes landed from gate findings (`e5933ad` AVKit remote commands, `922d956` AirPlay 2 pumping). **Phase 9 (UI audit + mandated docs + one pre-PR Codex review + PR #C) is next.** S3-3 / S3-4 still queued behind the S3-2 PR.
...
| S3-2 (pivot) | sequential | `avplayer-native-video-dsp` | `feat/avplayer-native-video-dsp` | C (not opened) | S3-1 merged ✅ + Phase 1 cherry-pick ✅ | `spike/avplayer-inplace-tap-dsp` ✅ done (branch still undeleted — close-out 10.3) | 🔧 **IMPLEMENTING** — branch HEAD `5125bb3` (2026-09-25), 87 commits ahead of `origin/main`, pushed to origin; unmerged; PR #C not yet opened. Steps 1-3 ✅; **Phases 1-7 ✅** Oracle-approved; **Phase 8 ✅ COMPLETE 2026-09-25** (all manual gates run with the user; 8.1b PARTIAL, 8.2/8.11/8.12 NOT ABLE, 8.8 N/A, rest PASS — `tasks/avplayer-native-video-dsp/verification.md`; 119 TSan tests, only pre-existing #86 fails); **Phase 9 NEXT.** See `_context/s3-2-pivot.md`. |
...
> Paths verified against code HEAD `056c69a` (2026-09-05; not re-verified against `5125bb3`). The S3-2 column is the **pivot** task `avplayer-native-video-dsp`, not the paused `vaer` branch; the pivot deliberately does **not** route video through `AVAudioEngine`, so `AudioEngineController.swift` is no longer a S3-2 conflict site.
...
**S3 progress:** Wave S3-1 ✅ COMPLETE (mwvi PR #80 + spt PR #82 both merged). S3-2 is now the pivot task `avplayer-native-video-dsp` (vaer PAUSED-AS-REFERENCE) — 🔧 IN PROGRESS: Phases 1-8 ✅ (Phase 8 complete 2026-09-25), **Phase 9 next**, branch pushed to origin at `5125bb3`; unmerged; PR #C not yet opened. S3-3 / S3-4 remain queued behind the S3-2 PR, and the post-S3 Structure Sprint behind all three.

**S3-2 pending items (everything downstream of S3 is gated on these) — as of 2026-09-05; the Phase 8 row is ✅ DONE as of 2026-09-25 (see "Phase 8 complete" in the Findings table below):**
...
**Known open discrepancy:** Phase 2's close is recorded as 85/85 tests in `_context/s3-2-pivot.md:111` and `tasks/avplayer-native-video-dsp/state.md:51`, but as 74/74 in that task's todo (74/74 vs 85/85 discrepancy — open item for Phase 9 pre-PR Oracle). Historical per-phase close counts inside dated blocks (72/85/89/92/93/98/103/110/115) are correct snapshots, as is 116/116 (2026-06-27); the **current** count is 119 tests under TSan, only the pre-existing #86 failing (2026-09-25).
