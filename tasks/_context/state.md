...
> **Date:** 2026-02-21
> **Updated:** 2026-04-30 (S3-2 Phase 0 + 1 + 2 + 3 ✅ **all complete**. Phase 3 wires `videoSourceNode` into `AudioEngineController` parallel to `streamSourceNode`, refactors `AudioPlayer.playTrack` video branch through an async `startVideoTrack` Task, and gives `VideoPlaybackController.loadVideo` an `audioTap:` parameter with a post-await player-identity guard. Two-tier stale defence: tap-identity at AudioPlayer level + player-identity at VideoPlaybackController level. 6 implementation commits + 3 regression-fix commits surfaced from real-video manual test (video display via dropping `@ObservationIgnored` from VideoPlaybackController.player; double-audio gate on volume slider; videoLoadTask defer-clear for resume after pause). Oracle review arc closed at **9.5/10** final (impl: 8.4 → 9.2 → 9.4; regression-fix: 7 → 8 → 9.5). 90/90 tests pass with TSan. Manual video verified: frame displays, single audio path, slider clean, EQ + spectrum analyzer respond. Milkdrop deferred to Phase 6 per plan §11.3 (intentional). **Phase 5 (tap-failure watchdog per plan §10) is next; Phase 4 is no-op per Phase 0 Path NONE.**)
> **Previous:** 2026-04-30 (S3-2 Phase 0 ✅ + Phase 1 ✅ + Phase 2 ✅ — `VideoAudioTap` MTAudioProcessingTap wrapper ships at 9.3/10; 17 commits on `feat/video-audio-engine-routing`; 84/84 tests pass; AudioConverter is load-bearing per Phase 0.)
...
| S3-2 | sequential | `video-audio-engine-routing` | `feat/video-audio-engine-routing` | C | S3-1 merged ✅ + Phase 0 ✅ + Phase 1 ✅ + Phase 2 ✅ + Phase 3 ✅ | `spike/vaer-av-drift-measurement` ✅ deleted | 🔧 **IN PROGRESS** — Phase 0/1/2/3 done; Phase 5 (tap-failure watchdog per plan §10) next; Phase 4 no-op |
...
| `video-audio-engine-routing` | ✅ existing | ✅ | ✅ | **9.4/10** (plan); **9.5/10** (Phase 1: 9.5; Phase 2: 9.3; Phase 3: 9.5 final after regression-fix arc) | 3 plan + 1+3+5 impl |
