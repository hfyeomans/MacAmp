...
| Xcode MCP evaluated — LLDB path UNCONFIRMED (2026-09-05) | Xcode's own MCP server (`mcp__xcode__*`) failed to connect directly in Claude's session (`CONNECTION_CLOSED`), but it is reachable through the xcodebuildmcp bridge (`xcodebuildmcp xcode-ide list-tools`, user terminal) and exposes **47 tools** — more than the three in `CLAUDE.md` (`GetBuildLog`, `XcodeListNavigatorIssues`, `RenderPreview`). Confirmed: `GetConsoleOutput` (stdout/stderr/OSLog of an Xcode-launched app, regex + severity filters — an alternative to `log stream` during the manual gates). The bridge then timed out (`BRIDGE_LIST_TIMEOUT` twice) before the list could be checked for launch/breakpoint/expression tools. Open item: user enumerates the full list; if debugger tools exist, an Xcode *Run* of the Release scheme attaches a debugger without the get-task-allow re-sign. Until then `xcodebuildmcp debugging` is the LLDB path. Note: the xcodebuildmcp daemon does not auto-start inside Claude's sandboxed shell — it starts normally in a user terminal. | Small | 🟡 PARTIALLY EVALUATED — user to enumerate `xcode-ide list-tools` |
