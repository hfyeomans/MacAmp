...
- ✅ Generate xcodeproj from `project.yml` via `xcodegen generate` when file structure changes (never edit pbxproj manually)
...
- **T3 MainWindow Layer Decomposition** (Feb 22, 2026) - Full implementation of layer subview pattern (PR #54, merged)
  ...
  - Xcodeproj: generated by XcodeGen from `project.yml` (`xcodegen generate`)
...
**XcodeGen project generation** (adopted Feb 2026): The `.xcodeproj` is now **generated** from `project.yml` using XcodeGen. It is gitignored and never edited manually.

```bash
# Regenerate xcodeproj after any file changes
xcodegen generate
```

**Why XcodeGen:** The previous approach of manually editing `project.pbxproj` (4 sections per file change) was error-prone and caused multiple build failures when file references drifted out of sync. Before that, `rm -rf xcodeproj && open Package.swift` was tried but opens in SwiftPM package mode (no app icon, no signing, no run config).

**Configuration files:**
- `project.yml` — XcodeGen spec (committed to git). Defines targets, signing, entitlements, dependencies, schemes.
- `Package.swift` — SwiftPM spec (committed to git). Defines dependencies and target structure.
- `MacAmpApp.xcodeproj/` — Generated output (gitignored). Never edit, never commit.

**XcodeGen uses folder-based discovery:** When you add/move/delete `.swift` files, just run `xcodegen generate` and the project automatically picks them up. No manual file reference management.

**Note:** XcodeBuildMCP does not have built-in xcodegen integration. It works with existing xcodeproj files. Run `xcodegen generate` before using XcodeBuildMCP build/test tools.
...
10. **Run `xcodegen generate`** when file structure changes — xcodeproj is generated from project.yml, never edit manually
